                                    ...
                                    int *coeff_l, int nsamples, int ch_mask)
{
    ...
    // Scale left and right channels
    pos = (ch_mask & DCA_SPEAKER_MASK_C);
    fdsp->vector_fmul_scalar(samples[DCA_SPEAKER_L],
                             samples[DCA_SPEAKER_L],
                             coeff_l[pos    ] * scale,
                             nsamples);
    fdsp->vector_fmul_scalar(samples[DCA_SPEAKER_R],
                             samples[DCA_SPEAKER_R],
                             coeff_r[pos + 1] * scale,
                             nsamples);
    ...
    for (spkr = 0; spkr <= max_spkr; spkr++) {
        ...
        if (*coeff_l && spkr != DCA_SPEAKER_L)
            fdsp->vector_fmac_scalar(samples[DCA_SPEAKER_L],
                                     samples[spkr],
                                     *coeff_l * scale,
                                     nsamples);

        if (*coeff_r && spkr != DCA_SPEAKER_R)
            fdsp->vector_fmac_scalar(samples[DCA_SPEAKER_R],
                                     samples[spkr],
                                     *coeff_r * scale,
                                     nsamples);
...
static int filter_core_frame(DCAContext *s, AVFrame *frame)
{
    int ret = ff_dca_core_filter_frame(&s->core, frame);
...
static int filter_hd_ma_frame(DCAContext *s, AVFrame *frame)
{
    AVCodecContext *avctx = s->avctx;
    DCAXllChSet *p = &s->xll.chset[0];
    ...
    if ((ret = ff_dca_xll_filter(&s->xll)) < 0)
        return ret;

    // Handle downmixing to stereo request
    if (s->request_channel_layout == DCA_SPEAKER_LAYOUT_STEREO
        && DCA_HAS_STEREO(s->xll.output_mask) && p->dmix_embedded
    ...
    if (request_mask != s->xll.output_mask) {
        ff_dca_downmix_to_stereo_fixed(&s->dcadsp, s->xll.output_samples,
    ...
    if (!s->exss.assets[0].one_to_one_map_ch_to_spkr) {
        if (s->exss.assets[0].representation_type == DCA_REPR_TYPE_LtRt)
            matrix_encoding = AV_MATRIX_ENCODING_DOLBY;
        else if (s->exss.assets[0].representation_type == DCA_REPR_TYPE_LhRh)
            matrix_encoding = AV_MATRIX_ENCODING_DOLBYHEADPHONE;
    } else if (request_mask != s->xll.output_mask && p->dmix_type == DCA_DMIX_TYPE_LtRt) {
        matrix_encoding = AV_MATRIX_ENCODING_DOLBY;
    }
...
static int validate_hd_ma_frame(DCAContext *s)
{
    DCAXllChSet *p = &s->xll.chset[0], *c;
    int i, ch;

    s->has_residual_encoded = 0;
    for (i = 0, c = s->xll.chset; i < s->xll.nactivechsets; i++, c++) {
        ...
        for (ch = 0; ch < c->nchannels; ch++) {
            ...
            if (ff_dca_core_map_spkr(&s->core, c->ch_remap[ch]) < 0) {
    ...
    if (s->has_residual_encoded || (s->packet & DCA_PACKET_RECOVERY)) {
        int rate = s->core.sample_rate;
        int nsamples = s->core.npcmblocks * DCA_PCMBLOCK_SAMPLES;
