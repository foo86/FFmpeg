...
static void av_cold force_lossy_output(DCAXllDecoder *s, DCACoreDecoder *core, DCAXllChSet *c)
{
    int band, ch;
    ...
    for (ch = 0; ch < c->nchannels; ch++) {
        if (!(c->residual_encode & (1 << ch)))
            continue;
        if (ff_dca_core_map_spkr(core, c->ch_remap[ch]) < 0)
            continue;
        c->residual_encode &= ~(1 << ch);
    }
}

static int combine_residual_frame(DCAXllDecoder *s, DCACoreDecoder *core, DCAXllChSet *c)
{
    int ch, nsamples = s->nframesamples;
    DCAXllChSet *o;

    if (c->freq != core->output_rate || nsamples != core->npcmsamples)
    ...
    for (ch = 0; ch < c->nchannels; ch++) {
        ...
        spkr = ff_dca_core_map_spkr(core, c->ch_remap[ch]);
        if (spkr < 0)
            return AVERROR(EINVAL);
...
int ff_dca_xll_filter(DCAXllDecoder *s)
{
    ...
    for (i = 0, c = s->chset; i < s->nactivechsets; i++, c++) {
        ...
        if (c->residual_encode != (1 << c->nchannels) - 1
            && (ret = combine_residual_frame(s, &dca->core, c)) < 0)
            return ret;
